...
function PageMap() {
    ...
    this.searchBox;
    this.hasMapClickListener;
}

PageMap.prototype = {
    hasLabel: function(boundaryObject) {
        return boundaryObject.label !== null && typeof boundaryObject.label !== "undefined"
    },
    clearMarkers: function(boundaryType) {
        ...
        for (; i >= 0; i--) {
            ...
            if (hasLabel(boundaryObject) === true)
    ...
    setHasClickListener: function(hasListener) {
        this.hasMapClickListener = hasListener;
    },
    setSearchBox: function (sb) {
        this.searchBox = sb;
    }
}
...
function bindModel(page) {
    ...
    // bindings for the address search input
    var input = document.getElementById("pac-input");
    page.setSearchBox(new google.maps.places.Autocomplete(input));
    google.maps.event.addListener(page.searchBox, "place_changed", function () {
        searchProviders(page);
    });

    // bindings for the project search input
...
$(document).ready(function () {
    ...
    // ----------- creates google map-----------------
    page.setMap(new google.maps.Map(document.getElementById('map-canvas'), {
...
// Returns: 
// Sets: 
function searchProviders(page) {
    var place = page.searchBox.getPlace();
...
function getBoundary(boundary, page) {
    if (boundary !== null && typeof boundary === "string") {
        var bounds = page.map.getBounds();
